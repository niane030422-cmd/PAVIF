...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, global_step: int, remove_previous_ckpt: bool = False):
        ...
        # ========== save config / tokenizer (rank 0 only) ==========
        if self.rank == 0:
        ...
        # ========== save hf model ==========
        # gather full state dict to rank 0 via official FSDP API
        full_state_dict_config = FullStateDictConfig(offload_to_cpu=True, rank0_only=True)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            with FSDP.state_dict_type(self.model, StateDictType.FULL_STATE_DICT, full_state_dict_config):
                state_dict = self.model.state_dict()
                # rank0_only=True: non-rank-0 processes get empty dict {}
                # the context manager handles barrier internally
        ...
        if self.rank == 0:
            ...
            # assign=True: required for meta tensor assignment (PyTorch >= 2.1)
            save_model = save_model.to_empty(device="cpu")
            save_model.load_state_dict(state_dict, assign=True)
            save_model.save_pretrained(hf_local_path)
            print(f"[rank-{self.rank}]: Saved hf_model to {os.path.abspath(hf_local_path)}.")

            del save_model

        del state_dict

        # wait for rank0 to finish saving hf model
        dist.barrier()
