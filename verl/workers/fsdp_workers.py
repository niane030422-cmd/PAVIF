    ...
    def __init__(
        ...
        if self._is_actor:
            ...
            if (
                ...
                raise ValueError(
        f"Global batch size per device ({self.config.actor.global_batch_size_per_device}) "
        f"must be divisible by the micro batch size per device for update ({self.config.actor.micro_batch_size_per_device_for_update})."
    )
            if (
