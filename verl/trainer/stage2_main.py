...
from ..workers.reward.instruction_generation import self_CustomRewardManager
from .config import PPOConfig
from .ray_trainer_selfverify import RayPPOTrainer, ResourcePoolManager, Role
...
def main_task(config: PPOConfig):
    ...
    reward_fn = self_CustomRewardManager(
        tokenizer=tokenizer, num_examine=1, compute_score=config.worker.reward.compute_score,
    )
    val_reward_fn = self_CustomRewardManager(
        tokenizer=tokenizer, num_examine=1, compute_score="instruction_val"
    )
...
def main():
    ...
    ray_ip = cli_args.pop("ray_ip", None)
