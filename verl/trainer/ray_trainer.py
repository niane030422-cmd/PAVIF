...
from .metrics import compute_data_metrics_stage1, compute_throughout_metrics, compute_timing_metrics, reduce_metrics

from ..utils.stage2_rollout import apply_prompt_template
...
class RayPPOTrainer:
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        for test_data in self.val_dataloader:
            ...
            # len(test_batch) =60
            reward_tensor, acc = self.val_reward_fn(test_batch,rollout_data_dir=self.config.trainer.rollout_data_dir, global_step=self.global_step)
#            reward_tensor,score_c,score_l = self.val_reward_fn(test_batch)
        ...
        # self_verify
        return {"val/reward": reward_score, "acc": sum(acc_lst)/len(acc_lst)}
    ...
    def fit(self):
        ...
        for _ in range(self.config.trainer.total_episodes):
            for batch_dict in self.train_dataloader:
                ...
                metrics.update(compute_data_metrics_stage1(batch=batch, use_critic=self.use_critic))
                print("")
