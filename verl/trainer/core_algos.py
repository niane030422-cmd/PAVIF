...
# this implementation only consider outcome supervision, where the reward is a scalar.
@torch.no_grad()
...
def compute_policy_loss(
    ...
    clip_ratio_c = 3.0
    pg_losses_standard = torch.maximum(pg_losses, pg_losses2)
    pg_losses_dual = torch.min(-advantages * clip_ratio_c, pg_losses_standard)
    pg_losses_final = torch.where(advantages < 0, pg_losses_dual, pg_losses_standard)

    pg_loss = VF.masked_mean(pg_losses_final, eos_mask)
